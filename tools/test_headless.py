...
assert case in ('campaign', 'escape', 'blackout', 'destroyed', 'starter')
...
for stage, args in [('create', ['--create', str(save), '--map-gen-seed', '45176']),
                    ('run', ['--benchmark', str(save), '--benchmark-ticks', '108060' if case == 'starter' else '27000', '--benchmark-runs', '1'])]:
    log = run / f'{stage}.log'
    with log.open('w') as output:
        result = subprocess.run(base + args, stdout=output, stderr=subprocess.STDOUT)
    text = log.read_text()
    if result.returncode or 'FAI TEST FAILED' in text or 'non-recoverable error' in text:
    ...
    if stage == 'run' and not outcome_verified and not any(marker in text for marker in (
        ...
        'power isolation and blackout defeat', 'core destruction defeat', 'within 30 minutes')):
